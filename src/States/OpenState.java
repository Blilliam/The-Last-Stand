...
public class OpenState extends BaseState {
	...
	public void draw(Graphics2D g2) {
		...
		//THIS IS WHERE EVERYTHING GETS DRAWN (in order of y value for layering)
		for (int i = 0; i < entities.size(); i++) {
			entities.get(i).draw(g2);
			if (gameObj.getHitBoxToggleButton().getState())
				((Entity) entities.get(i)).drawHitBox(g2);
		}

		for (int i = 0; i < gameObj.getDamageTexts().size(); i++) {
			DamageText e = gameObj.getDamageTexts().get(i);
			e.draw(g2, gameObj.getCameraX(), gameObj.getCameraY());
