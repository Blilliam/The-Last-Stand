 ...
 *
 * Expected audio files (place in project root "Audio/" folder):
 *   Audio/bgm.wav       – main gameplay loop
 *   Audio/boss.wav      – boss fight loop
 *
 * Both clips are looped. Crossfade runs on a background thread so the game
 * loop is never stalled.
 */
public class MusicManager {

    public enum Track { BGM, BOSS, NONE }
    ...
    public void playBGM() {
        if (currentTrack == Track.BGM) return;
    ...
    public void playBossMusic() {
        if (currentTrack == Track.BOSS) return;
    ...
    public void stopAll() {
        if (currentTrack == Track.NONE) return;
    ...
    /**
     * Fades `outClip` to silence while fading `inClip` from silence to full.
     * Runs on a daemon thread so it never blocks the game loop.
     */
    ...
    private float getVolume(Clip clip) {
        ...
        } catch (IllegalArgumentException e) { return masterVolume; }
    }
}