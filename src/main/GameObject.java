...
public class GameObject {
	...
	private void startGame() {
		...
		// 2. Reinitialize everything fresh
		...
		// 3. Rebuild core systems (order matters!)
		map = new Background(this);
		player = new Player(this);
		cam = new Camera(this); // ← reset camera so it tracks the NEW player
	...
	private void generateInteractibles() {
		...
		// -----------------------------------------------------------------
		// STEP 1: Spawn the Teleporter (Closer to the middle)
		// -----------------------------------------------------------------
		// Constrain it to an inner boundary (e.g., within 20% of the center)
		...
		// -----------------------------------------------------------------
		// STEP 2: Spawn Chests and Trishops (Chests more common)
		// -----------------------------------------------------------------
		int totalItems = 50; // Total items to spawn per stage

		for (int i = 0; i < totalItems; i++) {
			int spawnX = rand.nextInt(mapWidth);
			int spawnY = rand.nextInt(mapHeight);

			// 80% chance for a Chest, 20% chance for a TriShop
			if (/*rand.nextDouble() < 0.808*/ false) {
