...
public class GameObject {
	...
	private void generateInteractibles() {
		...
		for (int i = 0; i < totalItems; i++) {
			...
			if (rand.nextDouble() < 0.2) { // 20% chance for a Blood Shrine
				getInteractibles().add(new BloodShrine(this, spawnX, spawnY));
			} else if (rand.nextDouble() < 0.5) { // 40% chance for a Chest (if not a Blood Shrine)
				getInteractibles().add(new Chest(this, spawnX, spawnY));
			} else { // 40% chance for a TriShop (if not a Blood Shrine or Chest)
