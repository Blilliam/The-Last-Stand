...
public class GameButton {
	...
	private int x, y, w, h;
	private int arcWidth = 25;  // Controls horizontal corner roundness
	private int arcHeight = 25; // Controls vertical corner roundness
	...
	public boolean isHovering() {
		return MouseInput.getMouseX() >= x && MouseInput.getMouseX() <= x + w && MouseInput.getMouseY() >= y
				&& MouseInput.getMouseY() <= y + h;
	}
	...
	public void draw(Graphics2D g2) {
		...
		if (getImage() != null) {
			...
			int imgSize = h - 30; // Scale image to fit button height with some padding

			// If you want to keep original image size, use image.getWidth() and
			// image.getHeight()
			int imgX = x + padding;
			...
			textX = imgX + imgSize + padding; // Move text right of the image
		}
	...
	// Helper method to dynamically adjust the roundness if needed
	public void setCornerRoundness(int arcWidth, int arcHeight) {
