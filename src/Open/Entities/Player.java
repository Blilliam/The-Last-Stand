...
//		}
	...
	public void update() {
		...
		if (currHp <= 0) {
			isDead = true;
			gameObj.getMusicManager().stopAll();
			gameObj.getMusicManager().playSFX(SFX.DEATH);
			gameObj.getMusicManager().playBGM();
	...
	public void damage(int amount) {
		if (invincibilityFrames <= 0) {
			...
			gameObj.getMusicManager().playSFX(SFX.OnPlayerHit);
			...
			invincibilityFrames = (HIT_DELAY + extraInvulnFrames)/2;
			countAfterHit = 0;
	...
	public void drawStatSidebar(Graphics2D g2) {
		...
		// Values for stats array
		String flatDmgVal;
		int flatDmgFlag;
		if (flatDmg > 0) {
			flatDmgVal = "+" + (int) flatDmg;
			flatDmgFlag = 1;
		} else {
			flatDmgVal = "0";
			flatDmgFlag = 0;
		...
		int hpFlag;
		if (artifactFlatHp + bookHp > 0 || artifactPctHp > 0) {
			hpFlag = 1;
		} else {
			hpFlag = 0;
		}
		...
		int freeChestFlag;
		if (artifactManager.getPercentFreeChest() > 0) {
			freeChestFlag = 1;
		} else {
			freeChestFlag = 0;
		}
		...
		Object[][] rows = { { "§ OFFENSE", null, 0 }, { "Dmg Mult", String.format("×%.2f", totalDmgMult), 1 },
				{ "Flat Dmg", flatDmgVal, flatDmgFlag },
				{ "Crit Bonus", String.format("+%.0f%%", globalCritChance * 100), critFlag },
				{ "ThickCrit", String.format("%.0f%%", artifactManager.getThickCritChance() * 100), thickCritFlag },
				{ "Bonk", String.format("%.0f%%", bonkChance * 100), bonkFlag }, { "§ SPEED", null, 0 },
				{ "Atk Speed", String.format("-%.0f%%", totalASReduction * 100), asReductionFlag },
				{ "+Projectiles", projVal, projFlag },
		...
		for (Object[] row : rows) {
			...
			if (value == null) {
				...
				Color valColor;
				if (flag > 0) {
					valColor = new Color(100, 220, 130);
				} else if (flag < 0) {
					valColor = new Color(220, 90, 90);
				} else {
					valColor = new Color(200, 200, 200);
				}
	...
	public void drawMinimap(Graphics2D g2) {
		...
		g2.setColor(new Color(10, 10, 20, 100));
		...
		for (var interactible : gameObj.getInteractibles()) {
			...
			Color teleColor;
			if (isTeleporter) {
				teleColor = Color.CYAN;
			} else {
				teleColor = new Color(255, 215, 0, 200);
			}
			g2.setColor(teleColor);
			Color portalColor;
			if (isPortal) {
				portalColor = Color.GREEN;
			} else {
				portalColor = new Color(255, 215, 0, 200);
			}
			...
			if (isTeleporter) {
				...
				teleBlue = new Color(255, 255, 255, 120);
			}
			g2.setColor(teleBlue);
			if (isPortal) {
				portalColor = Color.GREEN;
			} else {
				portalColor = new Color(255, 215, 0, 200);
	...
	public void drawEndTimer(Graphics2D g2) {
		...
		int barWidth = 300;
		int barHeight = 50;
		int x = (AppPanel.WIDTH - barWidth) - 30;
		int y = AppPanel.HEIGHT / 2 - barHeight / 2 + 400;
