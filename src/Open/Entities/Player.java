...
//		artifactManager.addArtifact(new Anvil(gameObj));
		...
		for (int i = 0; i < 50; i++) {
			artifactManager.addArtifact(new Key(gameObj));
		}
	...
	private void drawStatSidebar(Graphics2D g2) {
		...
		double globalCritChance = bookCritBonus
				+ (artifactManager.getThickCritChance() > 0 ? artifactManager.getThickCritChance() : 0);
		...
		// Projectiles
		int bonusProj = artifactManager.getBonusProjectiles() + (int) getStatBonus("Quantity Over Quality Book");
