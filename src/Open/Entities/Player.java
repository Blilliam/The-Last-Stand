...
public class Player extends Entity {
	...
	private int nonHitCount;
	private int delayBeforeHealing = 180;
	...
	public Player(GameObject gameObj) {
		...
		// Set the initial hitbox frame directly to these coordinates
	...
	public void update() {
		...
		if (nonHitCount >= delayBeforeHealing && nonHitCount % 30 == 0) {
			currHp++;
			if (currHp > getMaxHp()) {
				currHp = getMaxHp();
			}
	...
	public void addOrUpgradeBook(Book newBook, WeaponRarity rarity) {
		Book existing = ownedBooks.get(newBook.getName());
		if (existing != null) {
			// Tome already owned — level it up by rarity amount
			int hpBefore = existing.getMaxHpBonus();
			existing.levelUp(rarity);
			int hpAfter = existing.getMaxHpBonus();
			if (hpAfter > hpBefore) {
				currHp += (hpAfter - hpBefore);
			}
		} else {
			// First acquisition — start at level 1 (rarity ignored for the first pick,
			// since level 1 is the base regardless)
			ownedBooks.put(newBook.getName(), newBook);
			if (newBook.getName().equals("Max HP Book")) {
				currHp += newBook.getMaxHpBonus();
			}
		}
	}
	...
	public double getMaxHp() {
		double bookBonus = ownedBooks.containsKey("Max HP Book") ? ownedBooks.get("Max HP Book").getMaxHpBonus() : 0;
		return (int) ((baseMaxHp + getArtifactManager().getFlatHealth() + bookBonus)
				* (1 + getArtifactManager().getPercentHealth()));
	}

	public void addExp(int i) {
		double bookBonus = ownedBooks.containsKey("EXP Book") ? ownedBooks.get("EXP Book").getExpBonus() : 0;
	...
	public void draw(Graphics2D g2) {
		...
		// 4. Render the sprite centered over the hitbox
	...
	private void drawGoldCounter(Graphics2D g2) {
		int barWidth = 200;
		int barHeight = 30;
		int hpX = (AppPanel.WIDTH - barWidth) / 2;
		int hpY = AppPanel.HEIGHT - barHeight - 100;

		// Sit directly above the HP bar with a small gap
		int coinSize = 28;
		int counterH = 30;
		int counterW = 110;
		...
		// Gold text — colour shifts red when too poor to buy anything
	...
	private void drawMinimap(Graphics2D g2) {
		...
		int mapY = AppPanel.HEIGHT - mapH - padding;
		...
		// Helper lambdas to convert world coords -> minimap coords
		// (defined as local variables using double[] trick for effectively-final
		// capture)
		final int fMapX = mapX, fMapY = mapY, fMapW = mapW, fMapH = mapH;

		// --- Interactibles (discovered = just draw them all as the player sees them)
		// ---
		for (var interactible : gameObj.getInteractibles()) {
			if (interactible.getState() == ChestState.OPEN)
				continue;

			int ix = fMapX + (int) ((double) interactible.getX() / worldW * fMapW);
			int iy = fMapY + (int) ((double) interactible.getY() / worldH * fMapH);
			// Clamp
			ix = Math.max(fMapX + 1, Math.min(fMapX + fMapW - 5, ix));
			iy = Math.max(fMapY + 1, Math.min(fMapY + fMapH - 5, iy));

			g2.setColor(new Color(255, 215, 0, 200)); // Gold for interactibles
			if (interactible.getClass().equals(Teleporter.class))
		...
		// --- Player Dot ---
		int px = fMapX + (int) ((double) x / worldW * fMapW);
		int py = fMapY + (int) ((double) y / worldH * fMapH);
		px = Math.max(fMapX + 2, Math.min(fMapX + fMapW - 6, px));
		py = Math.max(fMapY + 2, Math.min(fMapY + fMapH - 6, py));
