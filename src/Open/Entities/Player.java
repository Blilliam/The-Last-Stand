...
public class Player extends Entity {
	...
	public void update() {
		...
		((RectangularShape) this.getHitBox()).setFrame(this.x - width/2, this.y - height/2, this.width, this.height);
	...
	private void drawGoldCounter(Graphics2D g2) {
	    ...
	    if (coinAnimation.getFrame() != null) {
	        g2.drawImage(coinAnimation.getFrame(),
	                     cx + 4,
	                     cy + (counterH - coinSize) / 2,
	                     coinSize, coinSize, null);
	    }
	...
	private void drawMinimap(Graphics2D g2) {
	    ...
	    // (defined as local variables using double[] trick for effectively-final capture)
	    ...
	    for (var interactible : gameObj.getInteractibles()) {
	        if (interactible.getState() == ChestState.OPEN) continue;
