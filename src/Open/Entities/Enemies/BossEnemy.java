	...
	public void update() {
		...
		cloakFlare = (Math.sin(animTick * 1.5) + 1.0) / 2.0; // 0‒1
			...
			if (phase == 2) {
				...
				angleInc = 2;
			}			orbAngle = (orbAngle + angleInc) % 360;
...
//		));
	...
	private void drawCloak(Graphics2D g, int cx, int cy) {
		...
		Color cloakColor;
			if (phase == 2) {
				cloakColor = new Color(80, 0, 30);
			} else {
				cloakColor = new Color(30, 0, 60);
			}
	...
	private void drawCrown(Graphics2D g, int cx, int cy) {
		...
		Color gemColor;
			if (phase == 2) {
				gemColor = new Color(255, 60, 60);
			} else {
				gemColor = new Color(120, 0, 200);
			}
	...
	private void drawEyes(Graphics2D g, int cx, int cy) {
		...
		Color glowColor;
			if (phase == 2) {
				glowColor = new Color(255, 80, 0, 100);
			} else {
		...
		Color irisColor;
			if (phase == 2) {
				irisColor = new Color(255, 120, 0);
			} else {
				irisColor = new Color(0, 230, 255);
			}
