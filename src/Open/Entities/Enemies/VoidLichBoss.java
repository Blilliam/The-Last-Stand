 ...
 *
 * Phases: 1 (100%–60% HP) : Slow orbit of void orbs, fires 8-way spread 2
 * (60%–30% HP) : Faster, fires 12-way spread + spawns mini portals that shoot
 * back 3 (<30% HP) : "Collapse" — implodes toward player, fires 16-way spread,
 * orbs spiral inward
 *
...
public class VoidLichBoss extends Enemy {
	...
	private void fireVoidVolley() {
		...
		for (int i = 0; i < count; i++) {
			...
			shards.add(new VoidShard(x, y, Math.cos(angle) * speed, Math.sin(angle) * speed, size, false));
		}

		if (phase == 3) {
			for (int i = 0; i < 4; i++) {
				double angle = aimAngle + (Math.PI / 2.0 * i) + Math.PI / 4.0;
				shards.add(new VoidShard(x, y, Math.cos(angle) * (speed * 1.5), Math.sin(angle) * (speed * 1.5),
						size * 1.3f, false));
			}
		}
	}
	...
	private void drawVoidAura(Graphics2D g, int cx, int cy) {
		...
		for (int i = rings; i >= 1; i--) {
			...
			Color c;
			if (phase == 3) {
				c = new Color(180, 0, 255, Math.max(0, a));
			} else if (phase == 2) {
				c = new Color(120, 0, 200, Math.max(0, a));
			} else {
				c = new Color(60, 0, 140, Math.max(0, a));
			}
			g.setColor(c);
	...
	private void drawOrbRing(Graphics2D g, int cx, int cy) {
		...
		for (int i = 0; i < orbCount; i++) {
			...
			Color glow;
			if (phase == 3) {
				glow = new Color(255, 100, 255, 70);
			} else {
				glow = new Color(140, 60, 255, 60);
			}
			Color core;
			if (phase == 3) {
				core = new Color(230, 160, 255);
			} else {
				core = new Color(160, 100, 255);
			}
			Color pupil = new Color(30, 0, 60);

			g.setColor(glow);
			g.fillOval(ox - 12, oy - 12, 24, 24);
			g.setColor(core);
			g.fillOval(ox - 6, oy - 6, 12, 12);
			g.setColor(pupil);
	...
	private void drawCloak(Graphics2D g, int cx, int cy) {
		...
		Color cloakColor;
		if (phase == 3) {
			cloakColor = new Color(50, 0, 90);
		} else {
			cloakColor = new Color(20, 0, 50);
		}

		int[] fullX = { cx - 50, cx - 22, cx + 22, cx + 50, cx + 52 + hem, cx + 32, cx, cx - 32, cx - 52 - hem };
		int[] fullY = { cy - 10, cy - 40, cy - 40, cy - 10, cy + 75, cy + 85 + hem, cy + 80 + hem / 2, cy + 85 + hem,
				cy + 75 };
	...
	private void drawSkull(Graphics2D g, int cx, int cy) {
		Color skullColor;
		if (phase == 3) {
			skullColor = new Color(200, 180, 240);
		} else {
			skullColor = new Color(220, 215, 235);
		}
		...
		for (int tx : teethX) {
			g.fillPolygon(new int[] { tx, tx + 6, tx + 12 }, new int[] { cy - 44, cy - 34, cy - 44 }, 3);
		}
	...
	private void drawCrown(Graphics2D g, int cx, int cy) {
		...
		Color gemColor;
		if (phase == 3) {
			gemColor = new Color(255, 200, 255);
		} else {
			gemColor = new Color(220, 140, 255);
		}
	...
	private void drawEyes(Graphics2D g, int cx, int cy) {
		Color glowColor;
		if (phase == 3) {
			glowColor = new Color(255, 80, 255, 130);
		} else if (phase == 2) {
			glowColor = new Color(200, 80, 255, 110);
		} else {
			glowColor = new Color(140, 60, 255, 100);
		}
		Color irisColor;
		if (phase == 3) {
			irisColor = new Color(255, 150, 255);
		} else {
			irisColor = new Color(200, 120, 255);
		}
	...
	private void drawBossHPBar(Graphics2D g, int cx, int cy) {
		...
		Color fill;
		if (phase == 3) {
			fill = new Color(220, 80, 255);
		} else if (phase == 2) {
			fill = new Color(160, 50, 220);
		} else {
			fill = new Color(100, 30, 180);
		}
		g.setColor(fill);
	...
	public void damage(DamageResult result) {
		...
		if (bossCurrHp <= 0) {
			...
			for (int i = 0; i < 35; i++) {
				gameObj.addExp(8, (int) (x + (Math.random() * 100 - 50)), (int) (y + (Math.random() * 100 - 50)));
			}
		}
	}
