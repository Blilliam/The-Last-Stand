...
public class FireDemonBoss extends Enemy {
	...
	private void spawnFireballVolley() {
		...
		for (int i = 0; i < count; i++) {
			...
			fireballs.add(new Fireball(x, y, Math.cos(finalAngle) * speed, Math.sin(finalAngle) * speed, size));
		}
	}
...
//		g.draw(new Rectangle2D.Double(hitBox.getX() - gameObj.getCameraX(), hitBox.getY() - gameObj.getCameraY(),
//				hitBox.getWidth(), hitBox.getHeight()));
	...
	private void drawHead(Graphics2D g, int cx, int cy) {
		...
		for (int tx : teethX) {
			g.fillPolygon(new int[] { tx, tx + 5, tx + 10 }, new int[] { cy - 38, cy - 30, cy - 38 }, 3);
		}
	...
	public void damage(DamageResult result) {
		...
		if (bossCurrHp <= 0) {
			...
			for (int i = 0; i < 25; i++) {
				gameObj.addExp(5, (int) (x + (Math.random() * 80 - 40)), (int) (y + (Math.random() * 80 - 40)));
			}
		}
	}
