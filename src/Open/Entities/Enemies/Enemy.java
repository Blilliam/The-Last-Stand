...
public class Enemy extends Entity {
	...
	public void draw(Graphics2D g) {
		...
		BufferedImage img;
		if (isDying()) {
			img = deathAnim.getFrame();
		} else {
			img = walkAnim.getFrame();
		}
		if (img == null)
			return;
	...
	public void damage(DamageResult result) {
		...
		gameObj.getMusicManager().playSFX(SFX.OnEnemyHit);
	...
	private void die() {
		...
		int goldReward = (int) (value * goldMult);
		gameObj.getPlayer().addGold(goldReward);

		// Trigger artifact effects on kill (DemonSoul, etc.)
		gameObj.getPlayer().getArtifactManager().onKill(goldReward);
	}
