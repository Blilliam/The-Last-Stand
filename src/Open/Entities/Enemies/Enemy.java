...
public class Enemy extends Entity {
	...
	public Enemy(GameObject gameObj, int x, int y, int type, double statMultiplier) {
		...
		if ((maxHp / 10.0) * statMultiplier < 1) {
			this.value = 1;
		} else {
			this.value = (int) ((maxHp / 10.0) * statMultiplier);
		}
	}
	...
	private void die() {
		...
		gameObj.addExp((int) (value * (1 + gameObj.getPlayer().getArtifactManager().getPercentBonusExp())), x, y);
		if (Math.random() < gameObj.getPlayer().getArtifactManager().getBonusExpDropChance()) {
			gameObj.addExp((int) (value * (1 + gameObj.getPlayer().getArtifactManager().getPercentBonusExp())), x + 5, y + 5);
		}
		gameObj.getPlayer().addGold((int) (value * (1 + gameObj.getPlayer().getArtifactManager().getPercentBonusGold())));
	}
