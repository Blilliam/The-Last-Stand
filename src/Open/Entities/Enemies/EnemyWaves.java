...
public class EnemyWaves {
	...
	public EnemyWaves(GameObject gameObj) {
		...
		this.credits = 15; // Start with more credits for a faster early game
		this.creditGainRate = 0.30;
		creditGainRate *= gameObj.getDifficulty().ordinal() * 0.6; // Bonus credits based on difficulty
	...
	public double getDifficultyMult() {
		double value = (1.0 + (tickCounter / 1800.0) * 0.1) * gameObj.getMap().getStage()
				* gameObj.getDifficulty().ordinal();
		if (value < 1)
			value = 1; // Ensure it never goes below 1
		return value;
	}
	...
	public void update() {
		...
		if (tickCounter % 600 == 0) {
			creditGainRate += 0.03 * gameObj.getMap().getStage() * Math.pow(gameObj.getDifficulty().ordinal() + 1, 2); // Faster
																														// ramp
																														// up
		}
