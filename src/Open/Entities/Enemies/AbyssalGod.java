 ...
 *
 * ANTI-OP MECHANICS:
 *   1. REFLECT SHIELD  — periodic shield that sends damage back; spam-fire = suicide
 *   2. IMMUNITY PHASE  — must destroy "soul cores" around the boss to re-enable damage
 *   3. DEATH ZONES     — spreading void pools that linger; punishes standing still
 *   4. FORCED TELEPORT — if player stays >700px away for 3s, boss teleports on top of them
 *   5. HP DRAIN AURA   — passive %HP drain per second at close range
 *   6. DEATH NOVA      — explodes void shards on death; don't burst it from range
 *
 * Phases (by boss HP%):
 *   Phase 1 (100-70%): Slow, telegraphed attacks. Shield every 8s.
 *   Phase 2 (70-40%): Faster, soul cores appear, void pools start. Shield every 5s.
 *   Phase 3 (<40%):   Enrage — constant pressure, rapid teleport, HP drain, death nova on kill.
 */
public class AbyssalGod extends Enemy {
        ...
        void update(double bossX, double bossY) {
            ...
            for (var proj : gameObj.getProjectiles()) {
                if (!proj.isDead() && Entity.checkCollision(proj, new Entity(gameObj) {
                    { setHitBox(hitbox); this.x = (int)cx; this.y = (int)cy; width = 40; height = 40; }
                    public void update() {} public void draw(Graphics2D g) {}
                })) {
                    hp -= 5;
                    flashTimer = 6;
                    if (hp <= 0) dead = true;
    ...
    private static class AbyssalShard {
        ...
        AbyssalShard(double x, double y, double vx, double vy, float size, boolean homing) {
            ...
            this.size = size; this.homing = homing;
        }
        ...
        void update(double px, double py) {
            if (homing) {
                ...
                if (len > 0) { vx += (dx/len)*0.15; vy += (dy/len)*0.15; }
                double spd = Math.sqrt(vx*vx + vy*vy);
                if (spd > 6) { vx = vx/spd*6; vy = vy/spd*6; }
            }
            x += vx; y += vy; life--;
        }
    ...
    private void checkPhaseTransitions() {
        if (!p2Triggered && bossCurrHp < bossMaxHp * 0.70) {
            p2Triggered = true; phase = 2; phaseFlashTimer = 90;
            bossSpeed = Math.max(bossSpeed + 1, (int)(bossSpeed * 1.2));
            spawnCoreWave();
        }
        if (!p3Triggered && bossCurrHp < bossMaxHp * 0.40) {
            p3Triggered = true; phase = 3; phaseFlashTimer = 120;
            bossSpeed = Math.max(bossSpeed + 1, (int)(bossSpeed * 1.4));
            soulCores.clear(); immunityActive = false; // clear cores, then re-trigger
            spawnCoreWave();
        }
    }
    ...
    private void updateVoidPools() {
        ...
        if (poolSpawnCooldown >= interval) {
         ...
         while (it.hasNext()) {
             VoidPool vp = it.next();
             vp.life--;
             if (vp.life <= 0) { it.remove(); continue; }
     ...
     private void fireVolley() {
         ...
         for (int i = 0; i < count; i++) {
             ...
             shards.add(new AbyssalShard(x, y,
                 Math.cos(angle) * speed, Math.sin(angle) * speed, size, false));
         }
         ...
         if (addHoming) {
             for (int i = 0; i < 3; i++) {
                 ...
                 shards.add(new AbyssalShard(x, y,
                     Math.cos(a) * 2.5, Math.sin(a) * 2.5, 10f, true));
             }
         }
     }
     ...
     private void updateShards() {
         ...
         while (it.hasNext()) {
             ...
             s.update(px, py);
             if (s.isDead()) { it.remove(); continue; }
     ...
     private void spawnDeathNova() {
         ...
         for (int i = 0; i < count; i++) {
             ...
             shards.add(new AbyssalShard(x, y,
                 Math.cos(angle) * speed, Math.sin(angle) * speed, 16f, false));
         }
         // Also spawn lingering pools in a ring
         for (int i = 0; i < 8; i++) {
             double angle = (2 * Math.PI / 8) * i;
             voidPools.add(new VoidPool(
                 x + Math.cos(angle) * 200, y + Math.sin(angle) * 200, 100, 600));
         }
     }
     ...
     public void draw(Graphics2D g) {
         ...
         for (VoidPool vp : voidPools) vp.draw(g, gameObj.getCameraX(), gameObj.getCameraY());
         ...
         // Soul cores
         for (SoulCore c : soulCores) c.draw(g, x, y, gameObj.getCameraX(), gameObj.getCameraY());

         // Shards
         drawShards(g);

         if (!bossDying) drawHPBar(g, cx, cy + bob);
     ...
     private void drawBody(Graphics2D g, int cx, int cy) {
         ...
         for (int i = -2; i <= 2; i++) {
             ...
             g.fillPolygon(new int[]{tx-6, tx, tx+6}, new int[]{cy+96, cy+110+hemFlare+(i%2)*8, cy+96}, 3);
         }
     ...
     private void drawCrown(Graphics2D g, int cx, int cy) {
         ...
         for (int i = 0; i < 5; i++) {
             g.setColor(gold);
             g.fillPolygon(new int[]{bases[i]-8, bases[i], bases[i]+8}, new int[]{cy-108, cy-108-hgts[i], cy-108}, 3);
         }
    ...
    public void damage(DamageResult result) {
        ...
        if (bossCurrHp <= 0) {
            ...
            for (int i = 0; i < 50; i++) {
                gameObj.addExp(10,
                    (int)(x + (Math.random()*120 - 60)),
                    (int)(y + (Math.random()*120 - 60)));
            }
        }
    }

    @Override public boolean isDying() { return bossDying; }
