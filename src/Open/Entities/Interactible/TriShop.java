...
public class TriShop extends Interactible {
	...
	/**
	 * Calculate item price dynamically based on rarity and current cost multiplier
	 */
	private int getItemPrice(int idx) {
		Artifact art = items[idx];
		if (art == null) {
			return 0;
		}
		return (int) (getPrice(art.getRarity()) * getCostMultiplier());
	}
	...
	private void tryBuySlot(int idx) {
		...
		if (gold >= itemPrice || free) {
			...
			gameObj.incrementChestsOpened();
			
			// Trigger artifact effects on shop item opened (CreditCard, etc.)
			gameObj.getPlayer().getArtifactManager().onChestOpened();
		}
	}
	...
	private void drawSlot(Graphics2D g, int idx, int slotX, int slotY) {
		...
		boolean isHighlighted = (idx == closestSlot) && playerInRange && !isSold;
			...
			Color slotBg;
			if (isSold) {
				slotBg = new Color(30, 25, 15, 180);
			} else if (isHighlighted) {
				slotBg = new Color(45, 35, 15, 240);
			} else {
				slotBg = new Color(20, 15, 8, 220);
			}		g.setColor(slotBg);
		...
		if (isHighlighted) {
			...
			Color borderCol;
			if (isSold) {
				borderCol = new Color(60, 50, 30);
			} else {
				borderCol = new Color(140, 105, 40);
	...
	private void drawPriceTag(Graphics2D g, int idx, int slotX, int slotY) {
		int gold = gameObj.getPlayer().getGold();
		int itemPrice = getItemPrice(idx);
		boolean canAfford = gold >= itemPrice;
		...
		Color tagColor;
			if (canAfford) {
				tagColor = new Color(180, 140, 0);
			} else {
				tagColor = new Color(100, 30, 30);
			}
		g.setColor(tagColor);
		g.fillRoundRect(tagX, tagY, tagW, tagH, 4, 4);

			Color tagHighlightCol;
			if (canAfford) {
				tagHighlightCol = new Color(255, 215, 50);
			} else {
				tagHighlightCol = new Color(160, 60, 60);
			}			g.setColor(tagHighlightCol);
