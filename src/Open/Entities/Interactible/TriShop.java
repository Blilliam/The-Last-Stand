...
public class TriShop extends Interactible {
	...
	/**
	 * Fills the three slots with artifacts that are all different classes.
	 * Retries up to a safety limit to avoid an infinite loop if the pool is tiny.
	 */
	private void stockUniqueItems() {
		...
		for (int i = 0; i < ITEM_COUNT; i++) {
			...
			items[i] = candidate;
			prices[i] = candidate != null ? getPrice(candidate.getRarity()) : 0;
			sold[i] = false;
		}
	}
