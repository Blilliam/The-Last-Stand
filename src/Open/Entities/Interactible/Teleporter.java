...
import Open.Entities.Enemies.AbyssalGod;
...
public class Teleporter extends Interactible {
	...
	public Teleporter(GameObject gameObj, int x, int y) {
		...
		portalImage = Assets.Teleporter; // Placeholder, can be set to a specific image if desired
	}
	...
	public void open() {
		...
		switch (stage) {
		case 1 -> gameObj.getEnemies().add(new FireDemonBoss(gameObj, this, x, y, diff));
		case 2 -> gameObj.getEnemies().add(new VoidLichBoss(gameObj, this, x, y, diff));
		case 3 -> gameObj.getEnemies().add(new AbyssalGod(gameObj, this, x, y, diff));
		default -> gameObj.getEnemies().add(new FireDemonBoss(gameObj, this, x, y, diff));
		}
	...
	public void update() {
		...
		if (bossIsDefeated) {
			...
			gameObj.addExp(x, y, gameObj.getPlayer().getGold());
	...
	public void draw(Graphics2D g) {
		...
		g.drawImage(portalImage, drawX, drawY, width, height, null);
...
//		g.drawString(label, lx, ly);
	...
	public void setBossIsDefeated(boolean v) {
		this.bossIsDefeated = v;
	}
}