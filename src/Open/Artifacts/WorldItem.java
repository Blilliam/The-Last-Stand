    ...
    public void update() {
        if (isLanding) {
            ...
            if (Entity.rectCollision(this, gameObj.getPlayer())) {
                ...
                // Trigger artifact pickup notification
                gameObj.notifyArtifactPickup(artifact);
                
                gameObj.getMusicManager().playSFX(SFX.PickUp);
                
                floatTimer = 0; // Reset timer to reuse for pickup math
                if (artifact instanceof ChunkyOats)
                	gameObj.getPlayer().addHp(25);
            }
        } else {
