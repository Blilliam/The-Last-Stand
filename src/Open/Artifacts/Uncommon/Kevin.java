...
public class Kevin extends Artifact {
    ...
    public Kevin(GameObject gameObj) {
        super(gameObj);
        this.name = "Kevin";
        this.icon = Assets.UncommonDemonicBloodIcon; // reuse closest thematic icon
    ...
    // Called once per frame by ArtifactManager.onUpdate()
    @Override
    public void update() {
        if (damageThisTick) {
            applyKevinDamage();
    ...
    // Called by ArtifactManager.onKevinTrigger() whenever the player deals damage.
    // Sets a flag; damage is applied next update() so it runs on the game thread.
    public void triggerKevin() {
        tickCounter++;
        if (tickCounter >= 60) {   // throttle: at most once per second
            damageThisTick = true;
        }
    }

    private void applyKevinDamage() {
        double totalChance = 0.25 * count;
        int guaranteed = (int) totalChance;
        double partial  = totalChance - guaranteed;

        int dmg = guaranteed;
        if (Math.random() < partial) dmg++;
