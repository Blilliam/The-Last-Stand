...
public class ArtifactManager {
    ...
    public Artifact getRandomArtifact() {
        ...
        if (roll < 1)  return createNewInstance(legendaryPool);  //  1 % legendary
        if (roll < 20) return createNewInstance(uncommonPool);   // 19 % uncommon
        return createNewInstance(commonPool);                     // 80 % common
    }

    private Artifact createNewInstance(ArrayList<Artifact> pool) {
        if (pool.isEmpty()) return createNewInstance(commonPool);
    ...
    public void draw(Graphics2D g2) {
        ...
        for (Artifact artifact : artifacts) {
            ...
            if (x > AppPanel.WIDTH - 100) { x = startX; y += (displaySize + spacing); }
        }

        if (hoveredArtifact != null) drawArtifactTooltip(g2, hoveredArtifact, mx, my);
    }
    ...
    private void drawArtifactTooltip(Graphics2D g2, Artifact a, int mouseX, int mouseY) {
        ...
        for (String line : lines) maxTextWidth = Math.max(maxTextWidth, fmDesc.stringWidth(line));
    ...
    public double GetPercentDamage() {
        double out = 0;
        for (Artifact a : artifacts) out += a.getPercentDamage();
        return out;
    }

    public int GetFlatDamage() {
        int out = 0;
        for (Artifact a : artifacts) out += a.getFlatDamage();
        return out;
    }

    public int getBonusProjectiles() {
        int out = 0;
        for (Artifact a : artifacts) out += a.getBonusProjectiles();
        return out;
    }

    public double getPercentBonusGold() {
        double out = 0;
        for (Artifact a : artifacts) out += a.getPercentBonusGold();
        return out;
    }

    public double getPercentHealth() {
        double out = 0;
        for (Artifact a : artifacts) out += a.getPercentHealth();
        return out;
    }

    public int getFlatHealth() {
        int out = 0;
        for (Artifact a : artifacts) out += a.getFlatHealth();
        return out;
    }

    public void onCritEffect() {
        for (Artifact a : artifacts) a.onCritEffect();
    }

    public double getPercentAttackSpeed() {
        double out = 0;
        for (Artifact a : artifacts) out += a.getPercentAttackSpeed();
        return out;
    }

    public double getPercentLuck() {
        double out = 0;
        for (Artifact a : artifacts) out += a.getPercentLuck();
        return out;
    }

    /**
     * Free-chest chance from artifacts PLUS the Luck Book bonus.
     * Luck is currently expressed as an extra flat free-chest probability.
     */
    public double getPercentFreeChest() {
        double out = 0;
        for (Artifact a : artifacts) out += a.getPercentFreeChest();
    ...
    public double getPercentBonusExp() {
        double out = 0;
        for (Artifact a : artifacts) out += a.getPercentBonusExp();
        return out;
    }

    public double getBonusInvinsibilityFrames() {
        double out = 0;
        for (Artifact a : artifacts) out += a.getBonusInvinsibilityFrames();
        return out;
    }

    public double getBonusExpDropChance() {
        double out = 0;
        for (Artifact a : artifacts) out += a.getBonusExpDropChance();
        return out;
    }

    public void onUpdate() {
        for (Artifact a : artifacts) a.update();
    }
}