...
public abstract class Weapon {
	...
	public DamageResult getDmg() {
		...
		gameObj.getPlayer().getArtifactManager().onHitEffect();
	...
	public void update() {
		...
		if (delayCounter >= attackSpeed) {
			if (gameObj.getPlayer().closestEnemy(range) != null) {
				...
				double bookProjectileBonus = gameObj.getPlayer().getStatBonus("Quantity Book");
				projectilesToFire = (int) (stats.getOrDefault(WeaponUpgrades.ProjectileCount, 1.0)
