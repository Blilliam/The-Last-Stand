...
public abstract class Weapon {
    ...
    /**
     * Calculates damage for a single hit, factoring in all artifact and book bonuses.
     * Returns a DamageResult with the final number and a crit flag.
     */
    public DamageResult getDmg() {
        ...
        double baseDmg = (stats.getOrDefault(WeaponUpgrades.AttackDamage, 0.0)
                ...
                * (1 + gameObj.getPlayer().getArtifactManager().GetPercentDamage() + bookDamageBonus);
    ...
    public void update() {
        ...
        double attackSpeed = stats.getOrDefault(WeaponUpgrades.AttackSpeed, 100.0)
                * (1 - gameObj.getPlayer().getArtifactManager().getPercentAttackSpeed())
                * (1 - bookCooldownReduction);
        ...
        if (delayCounter >= attackSpeed) {
            if (gameObj.getPlayer().closestEnemy(range) != null) {
                ...
                projectilesToFire = stats.getOrDefault(WeaponUpgrades.ProjectileCount, 1.0).intValue()
                        + gameObj.getPlayer().getArtifactManager().getBonusProjectiles()
                        + bookProjectileBonus;
                delayCounter = 0.0;
            }
        }
    ...
    /**
     * AttackSize stat multiplied by the Size Book percentage bonus.
     * All projectile constructors should use this instead of reading
     * WeaponUpgrades.AttackSize directly.
     */
    ...
    /**
     * ProjectileSpeed stat multiplied by the Projectile Speed Book percentage bonus.
     * All projectile constructors that read ProjectileSpeed should use this instead.
     */
