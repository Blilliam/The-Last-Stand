...
public class DexecutionerProjectile extends WeaponEntity {
    ...
    public DexecutionerProjectile(GameObject gameObj, Weapon weapon, Vec2 direction, int x, int y) {
        ...
        double sizeMult = weapon.getStats().getOrDefault(WeaponUpgrades.AttackSize, 1.0) * 1.5;
    ...
    private void syncHitboxToPlayer() {
        // 1. Core player positioning center
        double pCenterX = gameObj.getPlayer().getX();
        double pCenterY = gameObj.getPlayer().getY();

        // Keep these if your parent entity math relies on them
        ...
        // 2. Mirror the exact visual polygon dimensions based on life progress
        ...
        // Create the local triangle spike matching the draw method
        int[] xPoints = {currentLength, 0, 0};
        int[] yPoints = {0, currentWidth / 2, -currentWidth / 2};
        Polygon localSpike = new Polygon(xPoints, yPoints, 3);

        // 3. Move and rotate the spike into world space
        AffineTransform tx = new AffineTransform();
        tx.translate(pCenterX, pCenterY); // Anchor at player center
        tx.rotate(this.angle);            // Rotate to point in attack direction

        // This is your final, accurate mathematical hitbox
