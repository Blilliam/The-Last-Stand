...
public class BananaProjectile extends WeaponEntity {
	...
	private double catchBackSpeed = 0.08;
	...
	public BananaProjectile(GameObject gameObj, Weapon weapon, Vec2 direction, int x, int y) {
		...
		this.velocity = direction.normalize().scale(speed * 1.5);
	...
	public void updatePhysics() {
		duration++;
		if ((duration >= 50 && Entity.checkCollision(this, gameObj.getPlayer())) || duration >= 500) {
			isDead = true;
		}
		...
		velocity = velocity.add(toPlayer.scale(catchBackSpeed));
		double maxSpeed = 7.0;
		if (velocity.length() > maxSpeed)
			velocity = velocity.normalize().scale(maxSpeed);
		
		catchBackSpeed = Math.min(11, catchBackSpeed + 0.003);
